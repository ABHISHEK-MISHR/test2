...
class _BottomBarBubbleExampleState extends State<BottomBarBubbleExample> {
  ...
  Future<void > getHealth() async {
    ...
    if( context.read<AuthProvider>().enteredOtp == "healthy working..."){
...
class BottomBarBubbleIconState extends State<BottomBarBubbleIcon> {
  ...
  @override
  void didUpdateWidget(covariant BottomBarBubbleIcon oldWidget) {
    super.didUpdateWidget(oldWidget);
    _isSelected = widget.isSelected;
  }

  void updateSelect(bool isSelected) {
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Icon(widget.item.icon, color: _isSelected ? widget.color : Colors.grey),
        Text(
          widget.item.label,
          style: TextStyle(color: _isSelected ? widget.color : Colors.grey),
        ),
      ],
    );
  }
